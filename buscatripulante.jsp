...
<html>
  ...
  <body>
    <%
      ...
      ResultSet listado = s.executeQuery ("SELECT COUNT(*) FROM tripulante WHERE nombre LIKE '%" + request.getParameter("nombre") + "%'");
      listado.next();
      ...
      ResultSet listado2 = s.executeQuery ("SELECT * FROM tripulante WHERE nombre LIKE '%" + request.getParameter("nombre") + "%'");

      // Si hay una única receta con el nombre especificado, vamos directamente
      // a la página de detalle.
      if (coincidencias == 1) {
        listado2.next();
        out.println("<script>document.location = \"detalleTripulante.jsp?IdTrip=" + listado2.getInt("IdTrip") + "\"</script>");
      }
            ...
            <div class="row">
              ...
              <div class="col s6 m6 l6">
                <div class="card grey lighten-5">
                  <h3 class="center">T R I P U L A N T E S</h3>
                  ...
                  while (listado2.next()) {
                    ...
                    <tr>
                      ...
                      <form method="post" action="detalleTripulante.jsp">
                        <input type="hidden" name="IdTrip" value="<%=listado2.getString("IdTrip") %>">
                        <td>
                        <button class="btn waves-effect waves-light center purple lighten-2" type="submit" name="editar">
                          <i class="fa fa-plus-circle"></i> Detalles
                        </button>
                        </td>
                      </form>
                    </tr>
